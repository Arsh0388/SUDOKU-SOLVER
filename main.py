...
class VarSelector:
    ...
    #     selector = available().select_variable(grid)
    #     return selector
    pass
    ...
    def select_variable(self, grid):
        ...
        for i in range(9) : 
            for j in range(9) : 
                ...
                if length <= min_domain and length > 1 : 
...
class AC3:
    ...
    def consistency(self, grid, Q):
        ...
        while  len(Q) != 0: 
            var = Q.pop() 
            assign = set()
            var_assigned1,bool_value = self.remove_domain_row(grid,var[0],var[1])
            if  bool_value  : 
                return False
            var_assigned2,bool_value  = self.remove_domain_column(grid,var[0],var[1]) 
            if  bool_value : 
                return False 
            var_assigned3,bool_value  = self.remove_domain_unit(grid,var[0],var[1])        
            if bool_value : 
                return False
            assign = (var_assigned1 + var_assigned2 + var_assigned3)
            
            # print(var_assigned)
            for var in assign : 
                Q.add(var)
            # Q.update(var_assigned)
        return True
...
class Backtracking:
    ...
    def search(self, grid, var_selector):
        ...
        if grid.is_solved() : 
            return True
        # the var_selector should be returning a the coordinates of the cell 
        var = var_selector.select_variable(grid) 
        ac2 = AC3()
        x,y = var[0],var[1]
        for d in grid.get_cells()[x][y] : 
            if grid.is_value_consistent(d,x,y) : 
                copy_grid = grid.copy()
                copy_grid.get_cells()[x][y] = d
                Q = set()
                Q.add((x,y))
                r_i = ac2.consistency(copy_grid,Q)
                if r_i :
...
for p in problems:
    ...
    var_selector2 = FirstAvailable()
    var_selector1 = MRV()
    run_consistency = AC3()
    run_consistency.pre_process_consistency(g)
    start = time.time() 
    result = backtrack.search(g,var_selector1)
    end = time.time()
    running_time_mrv.append(end - start)
    print("time taken by MRV ",(end - start))
    print('Is the current grid a solution? ', result)
    start = time.time() 
    result2 = backtrack.search(g,var_selector2)
    end = time.time()
    running_time_fa.append((end-start))
    print("time taken by FA ",(end - start))
    print('Is the current grid a solution? ', result2)
...
print(" total mrv time : ",sum(running_time_mrv))
print(" total_fa time : ",sum(running_time_fa))
plotter = PlotResults()
plotter.plot_results(running_time_mrv, running_time_fa,"Running Time Backtracking (MRV)","Running Time Backtracking (FA)", "running_time")
